...
const deprecationData = {
  ...
  ['COMPONENT_V_MODEL' /* COMPONENT_V_MODEL */]: {
    message: comp => {
      ...
      if (
        comp.props && shared.isArray(comp.props)
...
function emit(instance, event, ...rawArgs) {
  ...
  let handlerName
  let handler =
    props[(handlerName = shared.toHandlerKey(event))] ||
  ...
  if (!handler && isModelListener) {
    handler =
      props[(handlerName = shared.toHandlerKey(shared.hyphenate(event)))]
  }
  if (handler) {
...
function resolveInjections(
  injectOptions,
  ctx,
  checkDuplicateProperties = shared.NOOP
) {
...
function validateProps(rawProps, props, instance) {
  ...
  for (const key in options) {
    ...
    validateProp(
      ...
      opt,
      !shared.hasOwn(rawProps, key) &&
        !shared.hasOwn(rawProps, shared.hyphenate(key))
    )
  }
}
/**
