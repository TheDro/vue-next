...
import {
  createSimpleExpression as createSimpleExpression$1,
...
function parseUriParts(urlString) {
  ...
  return parse$1(isString(urlString) ? urlString : '', false, true)
}
  ...
  if (node.type === 1 /* ELEMENT */) {
    ...
    node.props.forEach((attr, index) => {
      ...
      node.props[index] = {
        type: 7 /* DIRECTIVE */,
        name: 'bind',
        arg: createSimpleExpression$1(attr.name, true, attr.loc),
        exp,
        modifiers: [],
        loc: attr.loc
      }
...
function getImportsExpressionExp(path, hash, loc, context) {
  if (path) {
    ...
    const exp = createSimpleExpression$1(name, false, loc, 2 /* CAN_HOIST */)
    context.imports.push({ exp, path })
    if (hash && path) {
      return context.hoist(
        createSimpleExpression$1(
          `${name} + '${hash}'`,
          false,
          loc,
          2 /* CAN_HOIST */
    ...
    return createSimpleExpression$1(`''`, false, loc, 2 /* CAN_HOIST */)
  }
}
        ...
        if (attr.name === 'srcset' && attr.type === 6 /* ATTRIBUTE */) {
          ...
          imageCandidates.forEach(({ url, descriptor }, index) => {
            if (
              ...
              const exp = createSimpleExpression$1(
                `"${url}"`,
                false,
                attr.loc,
                2 /* CAN_HOIST */
          ...
          node.props[index] = {
            type: 7 /* DIRECTIVE */,
            name: 'bind',
            arg: createSimpleExpression$1('srcset', true, attr.loc),
            exp: hoisted,
            modifiers: [],
            loc: attr.loc
          }
...
function compileScript(sfc, options) {
  ...
  if (!scriptSetup) {
    ...
    try {
      const scriptAst = parse$2(script.content, {
        plugins,
        sourceType: 'module'
      }).program.body
  ...
  function parse(input, options, offset) {
    try {
      return parse$2(input, options).program.body
    } catch (e) {
  ...
  for (const m of node.members) {
    if (m.type === 'TSPropertySignature' && m.key.type === 'Identifier') {
      props[m.key.name] = {
        ...
        required: !m.optional,
        type:
          process.env.NODE_ENV !== 'production' && m.typeAnnotation
